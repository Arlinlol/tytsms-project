 ...
 */
@Controller
public class MobileWapCartViewAction extends MobileWapBaseAction{
	@Autowired
	 ...
	 * @return
	 */
	private List<GoodsCart> cart_calc(String user_id, 
	 ...
	 * @return
	 */
	private List<GoodsCart> mobileWap_cart_calc(String user_id, 
			String cart_ids) {
		List<GoodsCart> carts_list = new ArrayList<GoodsCart>();// 用户整体购物车
		List<GoodsCart> carts_mobile = new ArrayList<GoodsCart>();// 未提交的用户mobile购物车
		List<GoodsCart> carts_user = new ArrayList<GoodsCart>();// 未提交的用户user购物车
		...
		String mobile_ids[] = cart_ids.split(",");
		for (String mobile_id : mobile_ids) {
			if (!mobile_id.equals("")) {
				mark_ids.add(Long.parseLong(mobile_id));
			}
		}
		if (user != null) {
			cart_map.clear();
			...
			cart_map.put("cart_status", 0);
			carts_mobile = this.goodsCartService
					.query("select obj from GoodsCart obj where obj.id in (:mark_ids) and obj.cart_status=:cart_status ",
							cart_map, -1, -1);
			// 如果用户拥有自己的店铺，删除carts_mobile购物车中自己店铺中的商品信息
			if (user.getStore() != null) {
				for (GoodsCart gc : carts_mobile) {
	 ...
	 * @param buy_type
	 */
	@RequestMapping("/mobileWap/add_goods_cart.htm")
	public void add_goods_cart(HttpServletRequest request,
		...
		if ("false".equals(map.get("login").toString())) {
			code = -100;
		}
		...
		if (code == 100) {
			...
			if (user != null) {
				...
				for (GoodsCart mobile : carts_mobile) {
					boolean addCartsMobile = true;
					for (GoodsCart gc2 : carts_user) {
						if (mobile.getGoods().getId()
								.equals(gc2.getGoods().getId())) {
							if (mobile.getSpec_info()
									.equals(gc2.getSpec_info())) {
								addCartsMobile = false;
								this.goodsCartService.delete(mobile.getId());
							}
						}
					}
					if (addCartsMobile) {// 将carts_mobile转变为user_cart
						mobile.setCart_mobile_id(null);
						mobile.setUser(user);
						this.goodsCartService.update(mobile);
						carts_list.add(mobile);
			...
			for (GoodsCart gc : carts_list) {
				if (gsp_ids != null && gsp_ids.length > 0
					...
					if (gc.getGoods()!=null && gc.getGoods().getId().toString().equals(goods_id)
		...
		int carts_size = carts_list.size() ;
		if(add){
			carts_size ++;
		}
		json_map.put("count", carts_size);
	 ...
	 * @param token
	 */
	@RequestMapping("/mobileWap/remove_goods_cart.htm")
	public void remove_goods_cart(HttpServletRequest request,
			HttpServletResponse response, String id ,String car_ids) {
		...
		if (id != null && !id.equals("")) {
			...
			carts = this.mobileWap_cart_calc(id , car_ids);
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/compute_cart.htm")
	 ...
	 * @param store_id
	 */
	@RequestMapping("/mobileWap/cart_count_adjust.htm")
	public void goods_count_adjust(HttpServletRequest request,
			HttpServletResponse response, String user_id , String gc_id, String count, String gcs) {
		...
		List<GoodsCart> carts = this.cart_calc(user_id , "");
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/goods_cart0.htm")
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/goods_cart1.htm")
	public ModelAndView goods_cart1(HttpServletRequest request,
			HttpServletResponse response, String token,
			String cart_mobile_ids) {
		
		ModelAndView mv = new JModelAndView("mobileWap/view/goods_cart1.html",
		...
		if(verify){
			...
			List<GoodsCart> carts = this.cart_calc(user_id, cart_mobile_ids);
			for (GoodsCart gc : carts) {
				Map map = new HashMap();
				...
				json_map.put("cart_price", CommUtil.formatMoney(cart_price));// 购物车总商品价格
				cart_list.add(map);
			}
			json_map.put("cart_list", cart_list);
			json_map.put("cart_list_size", cart_list.size());
		}
		mv.addObject("paymentTools", paymentTools);
		json_map.put("ret", CommUtil.null2String(verify));
		json_map.put("user_id", user_id);
		String json = Json.toJson(json_map, JsonFormat.compact());
		System.out.println("goodCar:json_map:"+ json_map);
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/goods_cart2.htm")
	public ModelAndView goods_cart2(HttpServletRequest request,
			HttpServletResponse response, String token,
			String cart_ids) {
		ModelAndView mv = new JModelAndView("mobileWap/view/goods_cart2.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		...
		if (cart_ids != null && !cart_ids.equals("")) {
			request.getSession(false).setAttribute("carts_gcs", cart_ids);
		} else {
			String session_gcs = CommUtil.null2String(request.getSession(false)
					.getAttribute("carts_gcs"));
			cart_ids = session_gcs;
		}
		List<GoodsCart> carts = this.cart_calc(user_id, cart_ids);
		boolean flag = true;
		...
		if (flag && carts.size() > 0) {
			...
			params.put("user_id",Long.parseLong(user_id));
			...
			mv.addObject("order_goods_price", this.calCartPrice(carts, cart_ids));
			...
			for (Object sl : store_list) {
				if (sl != "self" && !sl.equals("self")) {// 商家商品
					...
					if (gc_list != null && gc_list.size() > 0) {
						...
						Store  store = this.storeService.getObjById(CommUtil.null2Long(sl));
						map.put("store", store);
						store_ids += sl +"," ;
						map.put("store_goods_price",
								this.calCartPrice(gc_list, cart_ids));
			...
			mv.addObject("store_ids", store_ids);
		}else {
			flag = false ;
		}
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/goods_cart2_goodsInfo.htm")
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/goods_cart2_payment.htm")
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/goods_cart2_cartsTrans.htm")
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/goods_cart2_coupon.htm")
...
//										+ addr.getArea().getAreaName());
		...
		json_map1.put("all_of_price", all_of_price);
		json_map1.put("order_id", main_order_id);
		json_map1.put("order_num", order_num);
		json_map1.put("verify", verify);
		System.out.println("json_map1:" + json_map1);
		mv.addObject("json_map", json_map1);
	...
	@RequestMapping("/mobileWap/payment.htm")
	public ModelAndView payment(HttpServletRequest request,
			HttpServletResponse response,  String order_id) throws Exception {
		ModelAndView mv = new JModelAndView("mobileWap/view/goods_cart3.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		Map json_map = new HashMap();
		...
		if (user_verify) {// 用户信息验证通过
			order = this.orderFormService.getObjById(CommUtil
					.null2Long(order_id));
			all_of_price = order.getTotalPrice();
					...
					for (Map map : temp_maps) {
						OrderForm of = this.orderFormService
								.getObjById(CommUtil.null2Long(map
										.get("order_id")));
						all_of_price = all_of_price.add(of.getTotalPrice());
					}
				}
...
//			Map params = new HashMap();
//			params.put("mark", "wechatpay");
//			List<Payment> payments = this.paymentService.query(
//					"select obj from Payment obj where obj.mark=:mark", params,
//					-1, -1);
//			json_map1.put("payment_list", payments);
		}
		mv.addObject("paymentTools", paymentTools);
		json_map1.put("code", code);
		json_map1.put("order_id", order_id);
		json_map1.put("order_num", order.getOrder_id());
		json_map1.put("all_of_price", all_of_price);
		json_map1.put("verify", verify);
		System.out.println("json_map1:" + json_map1);
		mv.addObject("json_map", json_map1);
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/pay_payafter.htm")
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/pay_balance.htm")
							...
							for (Goods goods : goods_list) {
								...
								if (goods.getGroup_buy() == 2) {
									elasticsearchUtil.indexUpdate(IndexName.GOODS, IndexType.GOODS, 
											CommUtil.null2String(goods.getId()), IndexVoTools
											.groupGoodsToIndexVo(goods.getGroup_goods_list().get(goods
													.getGroup_goods_list()
													.size() - 1)));
//									String goods_lucene_path = ConfigContants.LUCENE_DIRECTORY
//											+ File.separator
...
//											.updateGoodsIndex(goods));
	 ...
	 * @return
	 */
	@RequestMapping("/mobileWap/pay_balance_verify.htm")
	 ...
	 * @param order_id
	 */
	@RequestMapping("/mobileWap/query_payment.htm")
	 ...
	 * @param order_id
	 */
	@RequestMapping("/mobileWap/query_payment_online.htm")
	 ...
	 * @param order_id
	 */
	@RequestMapping("/mobileWap/buyer_order_payment_save.htm")
	...
	@SecurityMapping(title = "订单支付", value = "/mobileWap/order_pay.htm*", rtype = "buyer", rname = "购物流程3", rcode = "goods_cart", rgroup = "在线购物")
	@RequestMapping("/mobileWap/order_pay.htm")
		...
		if (order != null && order.getOrder_status() == 10) {
			if (CommUtil.null2String(payType).equals("")) {
				...
				if (payType.equals("balance")) {// 使用预存款支付
					...
					mv = new JModelAndView("mobileWap/view/line_pay.html",
							configService.getSysConfig(),
							this.userConfigService.getUserConfig(), 1, request,
							response);
	...
	@RequestMapping("/mobileWap/order_address.htm")
	public void order_address(HttpServletRequest request,String gcIds,
			HttpServletResponse response, String addr_id, String store_id,String cart_ids) {
