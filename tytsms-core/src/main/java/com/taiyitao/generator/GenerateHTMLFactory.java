package com.taiyitao.generator;
...
public class GenerateHTMLFactory {
	 ...
	 */
	public void GenerateHTML(Map<String, Object> dataMap, String webappPath, String templatePath, String outputHTMLPath, String replacedHTMLPath) {
		try{
			//模板路径
			String path = "";
			String language = Strings.isEmpty(ConfigContants.I18N_LANGUAGE)?"zh_cn":ConfigContants.I18N_LANGUAGE.toLowerCase();
			if(ConfigContants.IS_UPLOAD_SERVER_MODEL){
				path =  ConfigContants.GENERATOR_FILES_SERVER_PATH+ File.separator + ConfigContants.GENERATOR_FILES_MIDDLE_NAME
						+ File.separator+language+File.separator;
			}else{
				 path = webappPath+ConfigContants.GENERATOR_FILES_MIDDLE_NAME+File.separator+"generator"+File.separator+language+File.separator;
			}
			System.out.println("模板路径="+path);
			File file = new File(path);
			if(!file.exists()){
				log.error("模板存放文件不存在，请检查该路径并确保模板存在该路径下，path="+path);
			}
			...
			File savefile = new File(outputHTMLPath);  
			...
			log.info("##########################outputFileName:" + outputHTMLPath);
			log.info("##########################replacedFileName:" + replacedHTMLPath);
			FileUtils.copyFile(new File(outputHTMLPath), new File(replacedHTMLPath));
		}catch(Exception e){
			log.error(e);
		}
	}
